...
/*   Updated: 2024/08/11 21:50:00 by bapasqui         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/cub3d.h"

void	draw_square2(t_mlx *mlx, int j, int i, int color)
{
	int	x;
	int	y;
	...
	while (x < (((j + 1) * 10)))
	{
		mlx_set_image_pixel(mlx->mlx, mlx->minimap->img, x + 5, (i * 10) + 5,
			color);
		x++;
	}
	while (y < (((i + 1) * 10)))
	{
		mlx_set_image_pixel(mlx->mlx, mlx->minimap->img, x + 5, y + 5, color);
		y++;
	}
}
