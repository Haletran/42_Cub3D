...
/*   Updated: 2024/07/28 22:33:20 by bapasqui         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void draw_debug(t_mlx *mlx, t_xy *xy, t_wh *wh)
{
    ...
    my_put_image(mlx, xy, wh, mlx->debug_box);
    mlx_string_put(mlx->mlx, mlx->win, xy->y + 10, xy->x + 20, 0x00FF0a42db, " DEBUG :");
    mlx_string_put(mlx->mlx, mlx->win, xy->y + 10, xy->x + 40, 0x00FFFFFFFF, ft_strjoin("x = ", ft_itoa(mlx->player->x)));
